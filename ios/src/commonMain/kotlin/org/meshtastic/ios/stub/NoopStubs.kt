...
package org.meshtastic.ios.stub
...
/**
 * No-op stub implementations for truly platform-specific interfaces on iOS.
 *
 * These stubs exist ONLY for interfaces that have no `commonMain` implementation and require platform-specific APIs
 * (CoreBluetooth transport, notifications, location services, etc.). All other interfaces use real `commonMain`
 * implementations wired through the generated Koin K2 modules.
 *
 * As real iOS implementations become available (e.g., BLE transport via Kable, CLLocationManager), they replace
 * individual stubs in [iosModule].
 */
private const val TAG = "IosNoopStub"

private fun logWarn(message: String) {
    Logger.w(tag = TAG) { message }
}

// region Transport / Radio Stubs
...
// region Notification / Platform Stubs
...
class NoopMeshServiceNotifications : MeshServiceNotifications {
    ...
    override fun updateServiceStateNotification(state: ConnectionState, telemetry: Telemetry?): Any = Unit
...
// region WorkManager / Location Stubs
...
// region Network Stubs (MQTT — not yet available on iOS)
