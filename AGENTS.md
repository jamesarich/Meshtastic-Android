...
-   **Networking:** Pure **Ktor** — no OkHttp anywhere. Engines: `ktor-client-android` for Android, `ktor-client-java` for desktop/JVM, `ktor-client-darwin` for iOS. Use Ktor `Logging` plugin for HTTP debug logging (not OkHttp interceptors). `HttpClient` is provided via Koin in `app/di/NetworkModule` and `core:network/di/CoreNetworkAndroidModule`.
...
-   **iOS ObjC class-property imports:** Kotlin/Native maps some ObjC class properties to **top-level Kotlin extension properties** rather than companion object members. These do not resolve without an explicit import even though the class itself is imported. Examples:
    -   `NSLocale.currentLocale` → requires `import platform.Foundation.currentLocale`
    -   `NSLocale.usesMetricSystem` (instance property) → requires `import platform.Foundation.usesMetricSystem`
    When a `platform.*` symbol appears to be "unresolved" despite the parent class being imported, check the Foundation klib directly: `strings ~/.konan/kotlin-native-prebuilt-macos-aarch64-*/klib/platform/ios_simulator_arm64/org.jetbrains.kotlin.native.platform.Foundation/default/linkdata/package_platform.Foundation/*.knm | grep <symbol>` to confirm its existence, then add a direct import.
    Additionally, `CPointer<ByteVarOf<Byte>>` arguments to C functions expecting `CValuesRef<UByteVarOf<UByte>>?` require `.reinterpret<UByteVar>()` after `addressOf(0)`. Import `kotlinx.cinterop.UByteVar` and `kotlinx.cinterop.reinterpret`.
