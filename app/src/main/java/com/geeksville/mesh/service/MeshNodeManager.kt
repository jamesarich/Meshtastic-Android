...
constructor(
    ...
    fun handleReceivedUser(fromNum: Int, p: User, channel: Int = 0, manuallyVerified: Boolean = false) {
        updateNodeInfo(fromNum) {
            ...
            if (shouldPreserve) {
                ...
                it.user = if (keyMatch) p else p.copy(public_key = NodeEntity.ERROR_BYTE_STRING)
    ...
    fun handleReceivedPosition(
        fromNum: Int,
        myNodeNum: Int,
        p: org.meshtastic.proto.Position,
    ...
    fun installNodeInfo(info: org.meshtastic.proto.NodeInfo, withBroadcast: Boolean = true) {
        updateNodeInfo(info.num, withBroadcast = withBroadcast) { entity ->
            val infoUser = info.user
            if (infoUser != null) {
                if (shouldPreserveExistingUser(entity.user, infoUser)) {
                    entity.longName = entity.user.long_name
                    entity.shortName = entity.user.short_name
                } else {
                    entity.user =
                        infoUser.copy(
                            public_key = if (infoUser.is_licensed) okio.ByteString.EMPTY else infoUser.public_key,
                            long_name = if (info.via_mqtt) "${infoUser.long_name} (MQTT)" else infoUser.long_name,
                        )
                    entity.longName = entity.user.long_name
                    entity.shortName = entity.user.short_name
                }
            }
            val infoPosition = info.position
            if (infoPosition != null) {
                entity.position = infoPosition
                entity.latitude = Position.degD(infoPosition.latitude_i ?: 0)
                entity.longitude = Position.degD(infoPosition.longitude_i ?: 0)
            }
            entity.lastHeard = info.last_heard
            val infoDeviceMetrics = info.device_metrics
            if (infoDeviceMetrics != null) {
                entity.deviceTelemetry = Telemetry(device_metrics = infoDeviceMetrics)
            }
